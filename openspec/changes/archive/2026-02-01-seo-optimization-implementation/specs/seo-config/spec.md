...
本规范定义了 Hagicode 文档站的搜索引擎优化(SEO)配置要求,确保站点在搜索引擎中具有最佳的可见性和可发现性。
...
- **高优先级**:
  - Sitemap 生成和配置
  - robots.txt 配置
  - 基础 meta 标签和 canonical URL
  - 结构化数据

- **中优先级**:
  - OG 标签和 Twitter Card 标签
  - 搜索引擎工具集成

- **低优先级**:
  - 高级结构化数据(如 BreadcrumbList)
  - 图片 sitemap
  - 视频结构化数据

### 兼容性

- 确保所有 SEO 配置与 Astro 5.16 兼容
- 确保与 Starlight 0.37.4 主题兼容
- 支持根路径和子路径两种部署模式
- 确保 SEO 配置不会破坏现有功能

### 性能考虑

- 结构化数据 JSON-LD 应该最小化(1-2KB per page)
- Sitemap 生成不应该显著增加构建时间
- Meta 标签不应该阻塞页面渲染
...
- [ ] `npm run build` 成功无错误
- [ ] `dist/sitemap.xml` 或 `dist/sitemap-index.xml` 存在且内容正确
- [ ] `dist/robots.txt` 存在且引用 sitemap
- [ ] 所有页面都有完整的 meta 标签
...
- [ ] `/sitemap.xml` 在浏览器中可访问且显示正确
- [ ] `/robots.txt` 在浏览器中可访问且格式正确
- [ ] OG 标签通过 Facebook Sharing Debugger
- [ ] Twitter Card 通过 Twitter Card Validator
- [ ] 结构化数据通过 Schema Markup Validator

### 搜索引擎验证

- [ ] 站点在 Google Search Console 验证通过
- [ ] 站点在 Bing Webmaster Tools 验证通过
- [ ] Sitemap 已提交到两个平台
- [ ] 搜索 `site:your-domain.com` 显示收录页面
